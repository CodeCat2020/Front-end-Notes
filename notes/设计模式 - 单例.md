...
- 不管使用这个类创建多少个新对象，新对象都会与第一次创建的对象完全相同。
- 将代码组织成一个逻辑单元，并可以通过单一变量进行访问。
...
- 用来划分命名空间，减少全局变量的数目。
- 把代码组织得更为一致，使其更容易阅读和维护。
- 只能被实例化一次。单体对象创建的命名空间是清除全局变量的最佳手段之一。



## 2、JavaScript 版

JavaScript 没有类，只有对象。新对象都是一个新的单体，因此，JavaScript 中永远不会有完全相等的对象，除非它们是同一个对象。

**每次使用对象字面量创建对象，实际上就是在创建一个单例**。
...
最简单的单体实际上就是一个对象字面量，把一批有一定关联的方法和属性组织在一起。
...
new Singleton();  // Uncaught TypeError: Singleton is not a constructor
```

所有成员可以通过变量 Singleton 使用圆点运算符访问。

在传统定义中，单体模式能被实例化一次。上例不是可实例化的类。为此，可以把单体模式定义得更广义一些：单体是一个用来划分命名空间并将批相关方法和属性组织在一 起的对象，如果它可以被实例化，那么它只能被实例化一次。



#### 单体模式的基本结构

单体模式只能实例化一次，后面再调用都是使用第一次实例化的结果。
...
当我们第一次调用构造函数，像往常一样返回 this，而后面再调用的话，都将重写构造函数，并访问私有变量`obj`并返回。
